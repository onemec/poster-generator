...
from utils import spotify_data_pull, rounded_rectangle, dominant_colors, image_from_url
import re
...
def get_font_by_lang(langid_classify, text_type):
    if text_type == "bold":
        if langid_classify[0] == 'ja':
            return fonts['NotoSansJP-Bold']
        elif langid_classify[0] == 'zh':
            return fonts['NotoSansTC-Bold']
        else:
            return fonts['OpenSans-Bold']
    elif text_type == "thin":
        if langid_classify[0] == 'ja':
            return fonts['NotoSansJP-Thin']
        elif langid_classify[0] == 'zh':
            return fonts['NotoSansTC-Thin']
        else:
            return fonts['SourceCodePro-Thin']


def create_spotify_code_url(theme, code_size, album_id) -> str:
    base_color = '101010' if theme == 'dark' else 'FFFFFF'
    text_color = 'white' if theme == 'dark' else 'black'
    return f'https://scannables.scdn.co/uri/plain/jpeg/{base_color}/{text_color}/{code_size}/spotify:album:{album_id}'
...
def generator(album, resolution, options) -> ImageDraw:
    ...
    #
    poster_theme = options['theme']
    if poster_theme == "dark":
    ...
    # define poster
    #
    album_poster = Image.new('RGBA', resolution, color=background_color)
    ...
    album_art_size = min(resolution[0] - 2 * spacing,
                         int((resolution[1] - 2 * spacing) * 0.6))  # full width but no larger than 60% of height
    ...
    album_poster.paste(album_art, (int(0.5 * resolution[0] - 0.5 * album_art_size), spacing), mask)
    ...
    #
    poster_draw = ImageDraw.Draw(album_poster, 'RGBA')

    # album artist
    max_text_length = int((resolution[0] - 2 * spacing) * 0.65)  # 65% of width is maximum
    artist_font_size = max_text_length // 9
    ...
    if text_length > max_text_length:
        ...
        artist_font = ImageFont.truetype(get_font_by_lang(data['album_artist'][1], "bold"),
                                         int(artist_font_size * reduce_factor))
    ...
    album_font_size = max_text_length // 27
    ...
    if text_length > max_text_length:
        reduce_factor = max_text_length / text_length  # calculate factor to get precise font size if too large
        album_font = ImageFont.truetype(get_font_by_lang(data['album_name'][1], "thin"),
                                        int(album_font_size * reduce_factor))
    ...
    playtime_font = ImageFont.truetype(fonts['SourceCodePro-Thin'], playtime_font_size)
    playtime_y_position = y_position + int(
        artist_font.size / 2.25) - playtime_font.size  # align playtime with bottom of album name instead of top
    poster_draw.text((resolution[0] - spacing - playtime_font.getbbox(data['playtime'])[2], playtime_y_position),
                     data['playtime'], text_color, font=playtime_font)
    ...
    for color in palette:
        poster_draw.rectangle(
            [x_posn, y_position, x_posn + (resolution[0] - 2 * spacing) / len(palette), y_position + line_height],
            fill=tuple(color), width=50)
    ...
    track_font = ImageFont.truetype(fonts['NotoSansJP-Thin'], track_font_size)
    track_line = ""
    for track in data['tracks']:
        ...
        if track_font.getlength(track_line) >= resolution[0] - spacing:
            track_line = track_line[:len(track_line) - len(f"{track} | ")]
    ...
    spotify_code_url = create_spotify_code_url(poster_theme, code_size, data["album_id"])
    spotify_code = image_from_url(url=spotify_code_url)

    if spotify_code is not None:
        code_scale = 0.16  # relative width of displayed spotify code
        spotify_code.resize(
            (int((resolution[0] - 2 * spacing) * code_scale), int((resolution[0] - 2 * spacing) * code_scale / 4)))
        code_width, code_height = spotify_code.size
        code_position = (
            resolution[0] - spacing // 2 - code_width,
            resolution[1] - spacing // 2 - code_height,
        )
        if poster_theme == 'dark':
        ...
        album_poster.paste(spotify_code, code_position)
    ...
    return album_poster, filename_friendly_album_name
